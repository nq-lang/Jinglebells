...
# ─────────────────────────────────────────────────────────────────────────────
def render_heston_page():
    _load_calibrated()
    T  = get_theme()
    hd = _CALIBRATED_HESTON
    if hd is None:
        st.error("Calibrated Heston data unavailable.")
        return

    _TC2 = get_theme()
    BG2, LINE_H, LINE2_H, TEXT2_H, TEXT3_H = (
        _TC2["chart_bg"], _TC2["chart_line"], _TC2["chart_line2"],
        _TC2["chart_t2"], _TC2["chart_t3"],
    )
    PLOTLY_H = dict(template="none", paper_bgcolor=BG2, plot_bgcolor=BG2,
                    font=dict(family="JetBrains Mono, monospace", color=TEXT2_H, size=10),
    ...
    <div style="display:flex;align-items:baseline;gap:14px;
      ...
      <span style="font-family:'Barlow Condensed',sans-serif;font-size:22px;
                   ...
                   color:{T['t1']};">HESTON IV SURFACE</span>
      <span style="font-family:'JetBrains Mono',monospace;font-size:10px;
                   color:{T['t3']};letter-spacing:1px;">
        Stochastic Volatility Model · Carr-Madan FFT · Calibrated ES/VIX 2014–2026
      </span>
    </div>""", unsafe_allow_html=True)

    # ── Parameter controls ──────────────────────────────────────────────────
    st.markdown(f"<div style='font-family:JetBrains Mono,monospace;font-size:8px;"
                f"color:{T['t3']};letter-spacing:2px;text-transform:uppercase;"
                f"margin-bottom:8px;'>Model Parameters · Sliders Trigger Recompute</div>",
                unsafe_allow_html=True)
    pc = st.columns(6)
    with pc[0]: v0    = st.slider("v₀ init var",   0.01, 0.50, float(hd["params"]["v0"]),   0.005)
    with pc[1]: kappa = st.slider("κ mean-rev",     0.10, 8.00, float(hd["params"]["kappa"]), 0.1)
    with pc[2]: theta = st.slider("θ long-run var", 0.01, 0.50, float(hd["params"]["theta"]), 0.005)
    with pc[3]: xi    = st.slider("ξ vol-of-vol",   0.05, 1.50, float(hd["params"]["xi"]),   0.01)
    with pc[4]: rho   = st.slider("ρ spot-vol",    -0.99,-0.01, float(hd["params"]["rho"]),   0.01)
    with pc[5]: r_h   = st.slider("r risk-free",    0.00, 0.10, float(hd["params"]["r"]),     0.005)

    params_live = {"v0": v0, "kappa": kappa, "theta": theta, "xi": xi, "rho": rho, "r": r_h}
    fok         = feller_condition(kappa, theta, xi)
    fc_col      = T["green"] if fok else T["red"]
    fc_txt      = f"✓ SATISFIED  2κθ={2*kappa*theta:.4f} > ξ²={xi**2:.4f}" if fok \
                  else f"✗ VIOLATED  2κθ={2*kappa*theta:.4f} < ξ²={xi**2:.4f}"

    st.markdown(f"""
    <div style="display:flex;gap:20px;flex-wrap:wrap;align-items:center;
                padding:8px 14px;background:{T['bg1']};border:1px solid {T['line2']};
                border-radius:4px;margin-bottom:14px;">
      <span style="font-family:JetBrains Mono,monospace;font-size:9px;
                   color:{fc_col};font-weight:700;letter-spacing:1px;">
        FELLER: {fc_txt}</span>
      <span style="font-family:JetBrains Mono,monospace;font-size:9px;color:{T['t3']};">
        Spot={hd['spot']:,.0f} &nbsp;·&nbsp; ATM 1M={hd['atm'][2]*100:.2f}%
        &nbsp;·&nbsp; 3M={hd['atm'][5]*100:.2f}%
        &nbsp;·&nbsp; 25Δ Skew 1M={(hd['skew'][2] or 0)*100:.2f}%
      </span>
    </div>""", unsafe_allow_html=True)

    # Decide whether to use calibrated or recomputed surface
    calibrated_p = hd["params"]
    use_calib = all(abs(params_live[k] - calibrated_p[k]) < 0.002
                    for k in calibrated_p)
    moneyness_g   = hd["moneyness"]
    expiry_days_h = [7, 14, 30, 45, 60, 90, 120, 180]
    expiry_lbls   = hd["expiries"]
    atm_idx       = 12  # moneyness=1.0

    if use_calib:
        surface_arr = np.array(hd["surface"], dtype=float)
        # surface already in pct
        surface_pct = np.where(np.isnan(surface_arr), np.nan, surface_arr)
    else:
        with st.spinner("Recomputing Heston surface via FFT…"):
            surface_pct = _heston_iv_grid(float(hd["spot"]), params_live,
                                          moneyness_g, expiry_days_h)

    # ── 3-D surface ─────────────────────────────────────────────────────────
    chart_col, panel_col = st.columns([7, 3])

    with chart_col:
        z_3d = surface_pct.tolist()
        v_min = float(np.nanmin(surface_pct))
        v_max = float(np.nanmax(surface_pct))

        colorscale_iv = [
            [0.00, T["red"]],   [0.20, "#CC1133"],
            [0.40, "#440011"],  [0.50, T["bg"]],
            [0.55, "#001A0A"],  [0.70, "#006633"],
            [0.85, "#00CC77"],  [1.00, T["green"]],
        ]
        fig3d = go.Figure(go.Surface(
            x=[f"{m*100:.0f}%" for m in moneyness_g],
            y=expiry_lbls, z=z_3d,
            colorscale=colorscale_iv,
            cmin=v_min, cmax=v_max,
            showscale=True, opacity=0.93,
            contours=dict(z=dict(show=True, usecolormap=True, project_z=False,
                                  size=max((v_max - v_min) * 0.07, 0.1))),
            colorbar=dict(
                title=dict(text="IV %", font=dict(size=9, color=TEXT3_H,
                           family="JetBrains Mono")),
                tickfont=dict(size=8, color=TEXT3_H, family="JetBrains Mono"),
                thickness=10, len=0.65,
            ),
            hovertemplate="Moneyness: %{x}<br>Expiry: %{y}<br>IV: %{z:.2f}%<extra></extra>",
            lighting=dict(ambient=0.55, diffuse=0.9, specular=0.1,
                          roughness=0.6, fresnel=0.05),
            lightposition=dict(x=100, y=200, z=600),
        ))
        # ATM term-structure line
        atm_z = [float(surface_pct[i, atm_idx])
                 if not np.isnan(surface_pct[i, atm_idx]) else 0
                 for i in range(len(expiry_lbls))]
        fig3d.add_trace(go.Scatter3d(
            x=["100%"] * len(expiry_lbls), y=expiry_lbls, z=atm_z,
            mode="lines+markers",
            line=dict(color=T["amber"], width=4),
            marker=dict(size=4, color=T["amber"]),
            hovertemplate="Expiry: %{y}<br>ATM IV: %{z:.2f}%<extra>ATM</extra>",
        ))
        _axis_style_h = dict(
            showgrid=True, gridcolor=LINE_H, gridwidth=1,
            showspikes=False, zeroline=False,
            tickfont=dict(size=7, color=TEXT3_H, family="JetBrains Mono, monospace"),
            backgroundcolor=BG2,
        )
        fig3d.update_layout(
            paper_bgcolor=BG2,
            ...
            margin=dict(l=0, r=0, b=0, t=0),
            scene=dict(
                xaxis=dict(title=dict(text="Moneyness", font=dict(color=TEXT3_H, size=9)),
                           **_axis_style_h),
                yaxis=dict(title=dict(text="Expiry", font=dict(color=TEXT3_H, size=9)),
                           **_axis_style_h),
                zaxis=dict(title=dict(text="IV %", font=dict(color=TEXT3_H, size=9)),
                           **_axis_style_h),
                camera=dict(eye=dict(x=1.7, y=-1.6, z=0.9),
                            up=dict(x=0, y=0, z=1)),
                dragmode="orbit",
                aspectmode="manual",
                aspectratio=dict(x=2.0, y=1.0, z=0.8),
            ),
            showlegend=False,
            font=dict(family="JetBrains Mono, monospace", color=TEXT2_H, size=10),
        )
        ...
        # ── 2D: Smile + term structure ──────────────────────────────────
        sm_col, ts_col = st.columns(2)
        smile_colors = [T["green"], T["amber"], T["red"], T["blue"], T["violet"]]

        with sm_col:
            fig_smile = go.Figure()
            for ei, lbl in enumerate(expiry_lbls[:5]):
                ys = [float(surface_pct[ei, j]) if not np.isnan(surface_pct[ei, j]) else None
                      for j in range(len(moneyness_g))]
                fig_smile.add_trace(go.Scatter(
                    x=[m * 100 for m in moneyness_g], y=ys,
                    mode="lines",
                    line=dict(color=smile_colors[ei], width=1.5),
                    name=lbl, connectgaps=True,
                    hovertemplate=f"{lbl} · %{{x:.0f}}% · IV: %{{y:.2f}}%<extra></extra>",
                ))
            fig_smile.add_vline(x=100, line_dash="dot", line_color=TEXT3_H,
                                line_width=1, annotation_text="ATM",
                                annotation_font_color=TEXT3_H, annotation_font_size=9)
            fig_smile.update_layout(
                **PLOTLY_H, height=220, showlegend=True,
                legend=dict(font=dict(size=8, family="JetBrains Mono"),
                            bgcolor="rgba(0,0,0,0)", orientation="h", y=1.08),
                xaxis=dict(title="Moneyness %", gridcolor=LINE_H,
                           tickfont=dict(size=9), range=[78, 122]),
                yaxis=dict(title="IV %", gridcolor=LINE_H, tickfont=dict(size=9)),
                margin=dict(t=30, r=10, b=40, l=48),
                title=dict(text="Volatility Smile by Expiry",
                           font=dict(size=10, color=TEXT2_H), x=0.02),
            )
            st.plotly_chart(fig_smile, use_container_width=True,
                            config={"displayModeBar": False})

        with ts_col:
            atm_ivs_ts = [float(surface_pct[i, atm_idx])
                          if not np.isnan(surface_pct[i, atm_idx]) else None
                          for i in range(len(expiry_lbls))]
            raw_skew = [hd["skew"][i] if (hd["skew"][i] and
                        str(hd["skew"][i]) != "nan") else None
                        for i in range(len(expiry_lbls))]
            skew_pct = [v * 100 if v is not None else None for v in raw_skew]

            fig_ts = make_subplots(specs=[[{"secondary_y": True}]])
            fig_ts.add_trace(go.Scatter(
                x=expiry_lbls, y=atm_ivs_ts, mode="lines+markers",
                line=dict(color=T["amber"], width=2),
                marker=dict(size=5, color=T["amber"]),
                name="ATM IV %",
                hovertemplate="%{x} · ATM IV: %{y:.2f}%<extra></extra>",
            ), secondary_y=False)
            fig_ts.add_trace(go.Bar(
                x=expiry_lbls, y=skew_pct, opacity=0.55,
                marker_color=[T["red"] if (v and v > 0) else T["green"]
                              for v in skew_pct],
                name="25Δ Skew %",
                hovertemplate="%{x} · Skew: %{y:.2f}%<extra></extra>",
            ), secondary_y=True)
            fig_ts.update_layout(
                **PLOTLY_H, height=220, showlegend=True,
                legend=dict(font=dict(size=8, family="JetBrains Mono"),
                            bgcolor="rgba(0,0,0,0)", orientation="h", y=1.08),
                margin=dict(t=30, r=20, b=40, l=48),
                title=dict(text="ATM Term Structure & 25Δ Skew",
                           font=dict(size=10, color=TEXT2_H), x=0.02),
            )
            fig_ts.update_yaxes(title_text="ATM IV %", secondary_y=False,
                                gridcolor=LINE_H, tickfont=dict(size=9))
            fig_ts.update_yaxes(title_text="25Δ Skew %", secondary_y=True,
                                tickfont=dict(size=9))
            fig_ts.update_xaxes(tickfont=dict(size=9), gridcolor=LINE_H)
    ...
    with panel_col:
        st.markdown(f"""
        <div class="kl-panel">
          <div class="kl-header">Calibration Summary</div>
          <div class="kl-row"><span class="kl-name">Period</span>
            <span class="kl-val" style="color:{T['t1']};">2014–2026</span></div>
          <div class="kl-row"><span class="kl-name">Underlying</span>
            <span class="kl-val" style="color:{T['t1']};">ES / SPX</span></div>
          <div class="kl-row"><span class="kl-name">Calib. spot</span>
            <span class="kl-val" style="color:{T['amber']};">${hd["spot"]:,.2f}</span></div>
          <div class="kl-row"><span class="kl-name">ATM IV 1W</span>
            <span class="kl-val" style="color:{T['green']};">{hd["atm"][0]*100:.2f}%</span></div>
          <div class="kl-row"><span class="kl-name">ATM IV 1M</span>
            <span class="kl-val" style="color:{T['green']};">{hd["atm"][2]*100:.2f}%</span></div>
          <div class="kl-row"><span class="kl-name">ATM IV 3M</span>
            <span class="kl-val" style="color:{T['green']};">{hd["atm"][5]*100:.2f}%</span></div>
          <div class="kl-row"><span class="kl-name">ATM IV 6M</span>
            <span class="kl-val" style="color:{T['green']};">{hd["atm"][7]*100:.2f}%</span></div>
          <div class="kl-row"><span class="kl-name">25Δ Skew 1M</span>
            <span class="kl-val" style="color:{T['red']};">{(hd["skew"][2] or 0)*100:.2f}%</span></div>
          <div class="kl-row"><span class="kl-name">ρ leverage</span>
            <span class="kl-val" style="color:{T['violet']};">{rho:.3f}</span></div>
          <div class="kl-row"><span class="kl-name">Feller 2κθ</span>
            <span class="kl-val" style="color:{fc_col};">{2*kappa*theta:.4f}</span></div>
          <div class="kl-row"><span class="kl-name">ξ²</span>
            <span class="kl-val" style="color:{T['t2']};">{xi**2:.4f}</span></div>
          <div class="kl-row"><span class="kl-name">v₀ → VIX equiv</span>
            <span class="kl-val" style="color:{T['amber']};">{(v0**0.5)*100:.1f}%</span></div>
          <div class="kl-row"><span class="kl-name">θ LR vol</span>
            <span class="kl-val" style="color:{T['amber']};">{(theta**0.5)*100:.1f}%</span></div>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div style="margin-top:12px;padding:11px 13px;background:{T['bg1']};
          ...
          <div style="font-family:JetBrains Mono,monospace;font-size:8px;
                      font-weight:700;color:{T['amber']};letter-spacing:2px;
                      text-transform:uppercase;margin-bottom:7px;">
            Mathematical Notes
          </div>
          <div style="font-family:Barlow,sans-serif;font-size:9.5px;
                      color:{T['t2']};line-height:1.75;">
            <b>Model:</b> Heston (1993) SV — dS = (r-q)S dt + √V S dW₁<br>
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;dV = κ(θ-V)dt + ξ√V dW₂<br>
            <b>Pricing:</b> Carr-Madan FFT via characteristic function inversion<br>
            <b>IV solve:</b> Brent root-finding on BS equation (xtol=1e-6, 200 iter)<br>
            <b>Feller:</b> 2κθ > ξ² → V(t) > 0 a.s. (no zero-variance paths)<br>
            <b>Leverage:</b> ρ < 0 captures SPX left skew via correlated BMs<br>
            <b>Skew:</b> d(ATM IV)/d(moneyness) ≈ ρξ/(2√θ) at long maturities
          </div>
        </div>
        """, unsafe_allow_html=True)
...
def _build_greek_matrix(raw_df: pd.DataFrame, spot: float, greek: str,
    ...
    """
    Build (strikes, expiries, Z-matrix) for a chosen Greek or metric.
    Returns (strike_list, expiry_list, np.ndarray) or (None, None, None).
    """
    if raw_df.empty or greek not in raw_df.columns and greek not in ("oi", "volume", "rel_intensity"):
        return None, None, None
    ...
    if greek == "rel_intensity":
        if "call_gex" in d.columns and "put_gex" in d.columns:
            ...
            d["gex_net_abs"] = d.get("gamma", pd.Series(0, index=d.index)) * d.get("open_interest", 1)
    ...
    # Limit to n_expiry nearest expiries
    all_exps = sorted(piv.columns)[:n_expiry]
    piv = piv[all_exps]

    return piv.index.tolist(), all_exps, piv.values
...
def render_greeks_3d_page():
    ...
    _TC3   = get_theme()
    BG3    = _TC3["chart_bg"]
    LINE3  = _TC3["chart_line"]
    LINE2_3 = _TC3["chart_line2"]
    TEXT2_3 = _TC3["chart_t2"]
    TEXT3_3 = _TC3["chart_t3"]
    PLOTLY3 = dict(template="none", paper_bgcolor=BG3, plot_bgcolor=BG3,
                   font=dict(family="JetBrains Mono, monospace",
                             color=TEXT2_3, size=10),
                   showlegend=False, hovermode="closest")
    ...
    <div style="display:flex;align-items:baseline;gap:14px;
      ...
      <span style="font-family:'JetBrains Mono',monospace;font-size:10px;
                   color:{T['t3']};letter-spacing:1px;">
        Gamma · Delta · Vanna · OI · Volume · Relative Intensity — Strike × Expiry
      </span>
    </div>""", unsafe_allow_html=True)

    asset_3d = st.session_state.asset_choice
    ctrl_c, _ = st.columns([2, 1])
    with ctrl_c:
        greek_choice = st.selectbox(
            "Surface metric",
            ["gamma", "delta", "vanna", "oi", "volume", "rel_intensity"],
            format_func=lambda x: {
                "gamma": "Γ  Gamma (dealer hedging pressure)",
                "delta": "Δ  Delta (directional exposure)",
                "vanna": "V  Vanna (vol × spot sensitivity)",
                "oi":    "OI  Open Interest",
                "volume": "Vol  Volume (today's flow)",
                "rel_intensity": "RI  Relative Intensity (|net GEX|)",
            }.get(x, x),
            label_visibility="collapsed",
        )

    greek_labels = {
        "gamma": "Γ Gamma", "delta": "Δ Delta", "vanna": "Vanna",
        "oi": "Open Interest", "volume": "Volume",
        "rel_intensity": "Relative Intensity",
    }
    ...
    with st.spinner(f"Fetching {asset_3d} options chain…"):
        try:
            ...
            raw_res = fetch_options_data(asset_3d, max_expirations=max_exp_3d)
            df_3d, spot_3d, raw_3d = raw_res
        except Exception as _e:
            st.error(f"Chain fetch error: {_e}")
            return

    if raw_3d.empty:
        st.warning("No options data available. Try another asset.")
        return

    # Compute gamma flip from main df
    try:
        gamma_flip_3d, cwall_3d, pwall_3d, _ = compute_key_levels(df_3d, spot_3d, raw_3d)
    except Exception:
        gamma_flip_3d = spot_3d

    strikes_3d, expiries_3d, Z_3d = _build_greek_matrix(raw_3d, spot_3d, greek_choice)
    if strikes_3d is None:
        st.warning(f"Could not build {greek_choice} matrix for {asset_3d}.")
        return

    Z_arr = np.array(Z_3d, dtype=float)
    if Z_arr.max() != Z_arr.min():
        Z_norm = (Z_arr - Z_arr.min()) / (Z_arr.max() - Z_arr.min())
    else:
        Z_norm = np.zeros_like(Z_arr)

    # Color scale per greek
    cs_map = {
        "gamma":         T["surface_colorscale"],
        "delta":         [[0,"#FF3860"],[0.5,T["bg"]],[1,"#00E5A0"]],
        "vanna":         [[0,"#9B7FFF"],[0.5,T["bg"]],[1,"#F5A623"]],
        "oi":            [[0,T["bg"]],[0.4,"#192D44"],[0.7,"#4A9FFF"],[1,"#C8DCEF"]],
        "volume":        [[0,T["bg"]],[0.4,"#1F3A58"],[0.7,T["amber"]],[1,T["t1"]]],
        "rel_intensity": [[0,T["bg"]],[0.3,"#7A0020"],[0.7,T["red"]],[1,T["amber"]]],
    }
    colorscale_3d = cs_map.get(greek_choice, T["surface_colorscale"])
    ...
    fig_3d = go.Figure(go.Surface(
        ...
        colorbar=dict(
            title=dict(text=greek_labels[greek_choice],
        ...
        hovertemplate=(f"Strike: %{{x:.2f}}<br>Expiry: %{{y}}<br>"
                       f"{greek_labels[greek_choice]}: %{{z:.4f}}<extra></extra>"),
        contours=dict(z=dict(show=True, usecolormap=True, project_z=False)),
        lighting=dict(ambient=0.55, diffuse=0.9, specular=0.12,
                      roughness=0.6, fresnel=0.05),
    ))
    # Spot & gamma flip vertical lines
    atm_nearest = min(strikes_3d, key=lambda k: abs(k - spot_3d))
    flip_nearest = min(strikes_3d, key=lambda k: abs(k - gamma_flip_3d))

    for lvl_k, lvl_col, lvl_name in [
        (atm_nearest, T["t1"], "SPOT"),
        (flip_nearest, T["amber"], "ZERO Γ"),
    ]:
        z_range = [float(Z_arr[strikes_3d.index(lvl_k), :].mean())] * len(exp_labels_3d)
        fig_3d.add_trace(go.Scatter3d(
            x=[lvl_k] * len(exp_labels_3d), y=exp_labels_3d, z=z_range,
            mode="lines",
            line=dict(color=lvl_col, width=3),
            hoverinfo="skip", name=lvl_name,
        ))
    ...
    fig_3d.update_layout(
        ...
        scene=dict(
            xaxis=dict(title=dict(text="Strike", font=dict(color=TEXT3_3, size=9)),
                       **_axis_style_3),
            yaxis=dict(title=dict(text="Expiry", font=dict(color=TEXT3_3, size=9)),
                       **_axis_style_3),
            zaxis=dict(title=dict(text=greek_labels[greek_choice],
                                  font=dict(color=TEXT3_3, size=9)),
                       **_axis_style_3),
            camera=dict(eye=dict(x=1.6, y=-1.7, z=0.9),
                        up=dict(x=0, y=0, z=1)),
            dragmode="orbit",
            aspectmode="manual",
            aspectratio=dict(x=2.5, y=0.8, z=0.7),
        ),
        showlegend=False,
        font=dict(family="JetBrains Mono, monospace", color=TEXT2_3, size=10),
        title=dict(
            text=f"{asset_3d} · {greek_labels[greek_choice]} Surface · Strike × Expiry",
            font=dict(size=11, color=TEXT2_3, family="JetBrains Mono, monospace"),
            x=0.01, xanchor="left",
        ),
    )
    ...
    # ── 2-D: Strike profile (net per expiry) ────────────────────────────────
    st.markdown(f'<div class="sec-head">{greek_labels[greek_choice]} Profile — Net Across All Expiries</div>',
                unsafe_allow_html=True)
    net_profile = Z_arr.sum(axis=1)
    bar_colors = [T["bar_pos"] if v >= 0 else T["bar_neg"] for v in net_profile]
    ...
    fig_profile = go.Figure(go.Bar(
        ...
        hovertemplate=f"Strike: %{{y:.2f}}<br>Net {greek_labels[greek_choice]}: %{{x:.5f}}<extra></extra>",
    ))
    fig_profile.add_hline(y=spot_3d, line_dash="solid", line_color=T["t1"],
                          line_width=1.5, annotation_text="  Spot",
                          annotation_font_color=T["t1"], annotation_font_size=9)
    fig_profile.add_hline(y=gamma_flip_3d, line_dash="dot", line_color=T["amber"],
                          line_width=1, annotation_text="  Zero Γ",
                          annotation_font_color=T["amber"], annotation_font_size=9)
    fig_profile.update_layout(
        **PLOTLY3, height=300,
        xaxis=dict(title=greek_labels[greek_choice], gridcolor=LINE3,
                   tickfont=dict(size=9), zerolinecolor=LINE2_3, zerolinewidth=1),
        yaxis=dict(title="Strike", gridcolor=LINE3,
                   range=[spot_3d * 0.92, spot_3d * 1.08], tickfont=dict(size=9)),
        margin=dict(t=10, r=80, b=40, l=60),
...
def _es_gex_levels_from_live(ticker: str = "SPY") -> dict:
    """Compute GEX key levels from live SPY chain for ES overlay."""
    _key = f"_es_gex_levels_{ticker}"
    _now = datetime.datetime.utcnow()
    _cached = st.session_state.get(_key)
    if _cached and (_now - _cached["ts"]).total_seconds() < 120:
        return _cached["data"]
    try:
        df_g, spot_g, raw_g = fetch_options_data(ticker, max_expirations=2)
        if df_g.empty:
            raise RuntimeError("empty")
        gflip, cwall, pwall, mpain = compute_key_levels(df_g, spot_g, raw_g)
        vtrig, mwall, _ = compute_intraday_levels(df_g, spot_g)
        # Convert SPY → ES equiv
        es_ratio = get_es_spy_ratio(spot_g) if ticker == "SPY" else 1.0
        result = {
            "spot":       spot_g * es_ratio,
            "gamma_flip": gflip  * es_ratio,
            "call_wall":  cwall  * es_ratio,
            "put_wall":   pwall  * es_ratio,
            "max_pain":   mpain  * es_ratio,
            "vol_trigger":vtrig  * es_ratio,
            "source":     ticker,
        }
        st.session_state[_key] = {"data": result, "ts": _now}
        return result
    except Exception:
        # Fall back to calibrated levels
        _load_calibrated()
        ld = _CALIBRATED_LEVELS
        return {
            "spot":       ld["spot"],
...
def render_es_chart_page():
    ...
    ld  = _CALIBRATED_LEVELS
    _TC_ES  = get_theme()
    BG_ES   = _TC_ES["chart_bg"]
    LINE_ES = _TC_ES["chart_line"]
    LINE2_ES = _TC_ES["chart_line2"]
    TEXT2_ES = _TC_ES["chart_t2"]
    TEXT3_ES = _TC_ES["chart_t3"]
    PLOTLY_ES = dict(template="none", paper_bgcolor=BG_ES, plot_bgcolor=BG_ES,
                     font=dict(family="JetBrains Mono, monospace",
                               color=TEXT2_ES, size=10),
    ...
    <div style="display:flex;align-items:baseline;gap:14px;
      ...
      <span style="font-family:'JetBrains Mono',monospace;font-size:10px;
                   color:{T['t3']};letter-spacing:1px;">
        Live Intraday · GEX Walls · VWAP · Global Session 18:00–16:00 ET
      </span>
    </div>""", unsafe_allow_html=True)
    ...
    with cc4:
        show_gex   = st.checkbox("GEX Walls", value=True)
    with cc5:
        show_vol   = st.checkbox("Volume",  value=True)

    # ── Data fetch ───────────────────────────────────────────────────────────
    with st.spinner("Loading ES data…"):
    ...
    if show_gex:
        with st.spinner("Computing GEX levels…"):
            gex_lvls = _es_gex_levels_from_live("SPY")

    # ── Fallback to calibrated H1 chart if live unavailable ──────────────────
    using_live = True
    if df_es.empty:
        using_live = False
        rows = ld.get("h1Chart", [])
        if rows:
            ...
            st.error("ES data unavailable. Ensure market hours or check network.")
            return
    ...
    if show_vwap and "volume" in df_es.columns:
        ...
        cv   = vol.cumsum()
        ctpv = (tp * vol).cumsum()
        df_es["vwap"] = np.where(cv > 0, ctpv / cv, tp)
    else:
        df_es["vwap"] = np.nan

    # ── Last known spot & price stats ─────────────────────────────────────────
    ...
    chg_color = T["green"] if chg_es >= 0 else T["red"]
    ts_label  = str(df_es["ts"].iloc[-1])[:16] if "ts" in df_es.columns else "—"
    src_label = "LIVE · Yahoo Finance" if using_live else "CALIBRATED · ES H1 2026"

    # ── Header stats strip ───────────────────────────────────────────────────
    st.markdown(f"""
    <div style="display:flex;gap:6px;margin-bottom:14px;flex-wrap:wrap;">
      <div style="flex:0 0 auto;background:{T['bg1']};border:1px solid {T['line2']};
                  border-top:2px solid {chg_color};border-radius:4px;
                  padding:10px 16px;">
        <div style="font-family:JetBrains Mono,monospace;font-size:8px;
                    color:{T['t3']};letter-spacing:2px;text-transform:uppercase;
                    margin-bottom:4px;">ES LAST · {ts_label}</div>
        <div style="font-family:'Barlow Condensed',sans-serif;font-size:28px;
                    font-weight:700;color:{T['t1']};letter-spacing:-1px;
                    line-height:1;">{spot_es:,.2f}</div>
        <div style="font-family:JetBrains Mono,monospace;font-size:12px;
                    color:{chg_color};margin-top:3px;">
          {chg_es:+.2f} &nbsp; ({chg_pct:+.2f}%)</div>
      </div>
      {''.join([
        f'<div style="flex:1;min-width:100px;background:{T["bg1"]};border:1px solid {T["line2"]};'
        f'border-radius:4px;padding:10px 14px;">'
        f'<div style="font-family:JetBrains Mono,monospace;font-size:8px;color:{T["t3"]};'
        f'letter-spacing:2px;text-transform:uppercase;margin-bottom:4px;">{lbl}</div>'
        f'<div style="font-family:JetBrains Mono,monospace;font-size:15px;'
        f'font-weight:500;color:{col};">{val}</div></div>'
        for lbl, val, col in [
    ...
    row_hts = [0.78, 0.22] if show_vol else [1.0]
    ...
    x_ts = df_es["ts"].dt.strftime("%H:%M").tolist() if "ts" in df_es.columns \
           else list(range(len(df_es)))
    ...
    fig_es.add_trace(go.Candlestick(
        ...
        hovertext=[
            ...
            for o, h, lo, c in zip(df_es["open"], df_es["high"],
    ...
    # ── GEX wall reference lines ──────────────────────────────────────────────
    if show_gex:
        level_defs = [
            ("gamma_flip", T["amber"],   "dot",   1.3, "Zero Γ"),
            ("call_wall",  T["green"],   "dash",  1.2, "Call Wall"),
            ("put_wall",   T["red"],     "dash",  1.2, "Put Wall"),
            ("vol_trigger",T["blue"],    "dashdot",1.0,"Vol Trigger"),
        ]
        y_min = float(df_es["low"].min())
        y_max = float(df_es["high"].max())
        for key, color, dash, width, label in level_defs:
            val = gex_lvls.get(key, None)
            if val is None:
                continue
            if not (y_min * 0.97 <= val <= y_max * 1.03):
                continue
            fig_es.add_hline(
                y=val, row=1, col=1,
    ...
    # ── Spot line ─────────────────────────────────────────────────────────────
    fig_es.add_hline(
        y=spot_es, row=1, col=1,
        line_dash="solid", line_color=T["t1"], line_width=1.5,
        annotation_text=f" LAST {spot_es:,.2f}",
        annotation_position="right",
        annotation_font_color=T["t1"], annotation_font_size=9,
    )
    ...
    if show_vol and "volume" in df_es.columns:
        ...
        fig_es.add_trace(go.Bar(
            ...
            marker=dict(color=vol_colors, line=dict(width=0), opacity=0.65),
            name="Volume",
            hovertemplate="Vol: %{y:,.0f}<extra></extra>",
        ), row=2, col=1)

    # ── Layout ────────────────────────────────────────────────────────────────
    y_pad = (float(df_es["high"].max()) - float(df_es["low"].min())) * 0.04
    y_lo  = float(df_es["low"].min())  - y_pad
    y_hi  = float(df_es["high"].max()) + y_pad
    ...
    fig_es.update_layout(
        **PLOTLY_ES,
        ...
        bargap=0.1,
        margin=dict(t=10, r=110, b=40, l=60),
        hoverlabel=dict(
            bgcolor=T["chart_hover"],
            bordercolor=T["chart_hover_border"],
            font=dict(family="JetBrains Mono", size=10,
                      color=T["chart_hover_text"]),
        ),
    )
    fig_es.update_xaxes(
        showgrid=True, gridcolor=LINE_ES, gridwidth=1,
        tickfont=dict(size=9, family="JetBrains Mono", color=TEXT3_ES),
        zerolinecolor=LINE2_ES,
        showspikes=True, spikesnap="cursor", spikemode="across",
        spikethickness=1, spikecolor=TEXT3_ES, spikedash="dot",
        row=1, col=1,
    )
    if show_vol:
        fig_es.update_xaxes(
            showgrid=True, gridcolor=LINE_ES,
            tickfont=dict(size=9, family="JetBrains Mono", color=TEXT3_ES),
            row=2, col=1,
        )
    fig_es.update_yaxes(
    ...
    if show_vol:
        fig_es.update_yaxes(
            showgrid=True, gridcolor=LINE_ES,
            tickfont=dict(size=8, family="JetBrains Mono", color=TEXT3_ES),
            row=2, col=1,
        )
    ...
    # ── Session stats table ───────────────────────────────────────────────────
    if show_gex:
        st.markdown(f'<div class="sec-head">Active GEX Levels — ES Equivalent</div>',
                    unsafe_allow_html=True)
        lvl_rows = [
            ("Gamma Flip (Zero Γ)", gex_lvls.get("gamma_flip"), T["amber"],
             "Dealer net gamma = 0. Above = long gamma (stable). Below = short gamma (volatile)."),
            ("Call Wall",           gex_lvls.get("call_wall"),  T["green"],
             "Peak positive GEX. Heavy dealer short-call hedging. Strong resistance."),
            ("Put Wall",            gex_lvls.get("put_wall"),   T["red"],
             "Peak negative GEX. Dealers long puts, short delta. Strong support."),
            ("Vol Trigger",         gex_lvls.get("vol_trigger"),T["blue"],
             "Max absolute vol-weighted GEX. Cross = intraday vol regime shift."),
            ("Spot",                gex_lvls.get("spot", spot_es), T["t1"],
             "Current ES level (or SPY-derived ES equivalent)."),
        ]
        cols_lvl = st.columns([2, 2, 4])
        with cols_lvl[0]:
            st.markdown(f"<span style='font-family:JetBrains Mono;font-size:8px;"
                        f"color:{T['t3']};letter-spacing:2px;'>LEVEL</span>",
                        unsafe_allow_html=True)
        with cols_lvl[1]:
            st.markdown(f"<span style='font-family:JetBrains Mono;font-size:8px;"
                        f"color:{T['t3']};letter-spacing:2px;'>PRICE</span>",
                        unsafe_allow_html=True)
        with cols_lvl[2]:
            st.markdown(f"<span style='font-family:JetBrains Mono;font-size:8px;"
                        f"color:{T['t3']};letter-spacing:2px;'>SIGNIFICANCE</span>",
                        unsafe_allow_html=True)
        for name, val, col, desc in lvl_rows:
            if val is None:
                continue
            dist = val - spot_es
            c1, c2, c3 = st.columns([2, 2, 4])
            with c1:
                st.markdown(f"<span style='font-family:JetBrains Mono;font-size:10px;"
                            f"color:{col};font-weight:600;'>{name}</span>",
                            unsafe_allow_html=True)
            with c2:
                _dc = T["green"] if dist > 0 else T["red"]
                st.markdown(
                    f"<span style='font-family:JetBrains Mono;font-size:11px;"
                    f"color:{col};font-weight:600;'>{val:,.2f}</span>"
                    f"<span style='font-family:JetBrains Mono;font-size:9px;"
                    f"color:{_dc};margin-left:6px;'>{dist:+.0f}</span>",
                    unsafe_allow_html=True)
            with c3:
                st.markdown(f"<span style='font-family:Barlow,sans-serif;font-size:9.5px;"
                            f"color:{T['t2']};'>{desc}</span>",
                            unsafe_allow_html=True)
...
_asset_options = ["SPY","QQQ","IWM","GLD","SLV","TLT","XLF","XLE","IBIT",
                  "SPX","NDX","RUT","AAPL","NVDA","TSLA","AMZN","MSFT","META","GOOGL"]
...
_PAGES = {
    "gex":        "📊  GEX DASHBOARD",
    "heston":     "📈  HESTON IV SURFACE",
    "greeks3d":   "🧊  3-D GREEK SURFACES",
    "es_chart":   "🕯️   ES FUTURES CHART",
...
def dashboard():
            ...
            if _landscape_fig is not None:
                st.plotly_chart(
                    ...
                    config={
                        ...
                        "modeBarButtonsToAdd": ["orbitRotation", "pan3d"],
                    }
                )
    ...
    # ── HEATMAP ───────────────────────────────────────────────────────────
        ...
        def _toggle_heat_dual():
            st.session_state.heat_dual = not st.session_state.get("heat_dual", False)
        st.button(_label, key="heat_dual_btn", on_click=_toggle_heat_dual)
